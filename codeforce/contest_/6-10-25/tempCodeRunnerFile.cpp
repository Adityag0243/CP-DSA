            ...
            for( int used = 0; used <= 1; used++){
                ...
                for( int steps = 0; steps <= k ; steps++){
                    if( !used) {
                        ...
                        if( steps == 0) continue;
                        // if( steps == 1){
                        //     // cout << "I am here ... " << v[i-1][j] << " " << v[i][j] << " .. "  << endl;
                        // }

                        dp[i][j][1][steps] = max({ dp[i][j][1][steps], v[i-1][j] + v[i][j] + dp[i][j][0][steps-1]  });
                        // cout << dp[i][j][1][steps] << "  ......\n";
                        if( j > 0){
                            dp[i][j][1][steps] = max({ dp[i][j][1][steps], v[i][j-1] + v[i][j] + dp[i][j][0][steps-1]  });
                        }
                    }
